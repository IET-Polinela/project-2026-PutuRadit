...
class MainAppMonolithicViewsCoverageTests(TestCase):
    ...
    def test_report_detail_api_valid(self):
        from reports.views import report_detail_api
    ...
    def test_report_search_unauthenticated(self):
        response = self.client.get(reverse('report_search_api') + '?q=Lampu')
    ...
    def test_report_create_view_admin_get(self):
        ...
        response = self.client.get(reverse('report_add'))
        self.assertIn(response.status_code, [200, 302])
        self.assertTemplateUsed(response, 'reports/add_report.html')
    ...
    def test_report_detail_view_unauthenticated(self):
        response = self.client.get(reverse('report_detail_api', kwargs={'id': self.report.id}))
        self.assertIn(response.status_code, [200, 302])
    ...
    def test_report_delete_view_direct_delete_method(self):
        from reports.views import ReportDeleteView
        from django.test import RequestFactory
